...
export const ExerciseQcm: React.FC<{ 
  ...
  const correctAnswers = question["items"].map((item) => { return item; });
  ...
  const validateButton = () => {
    if (selectedAnswer) {
      setSubmitted(true);
      if (question['answers'].includes(correctAnswers.indexOf(selectedAnswer))) {
        setCorrect(true);
  ...
  return (
    <div className={'flex justify-center'}>
      <div className={'flex flex-col links-exercice'}>
        ...
        {isSubmitted && actualQuestion + 1 < json.length &&
          <div className='mt-5'>
            ...
            <button onClick={() => {
              ...
              <p className='text-lg bg-slate-400 p-1 mt-5 dark:bg-slate-200 dark:text-black rounded'>Suivant</p>
            </button>
          </div>
        }
        {isSubmitted && actualQuestion + 1 === json.length &&
          <div>
            <p className={isCorrect ? "text-green-400" : "text-red-600" }>{ isCorrect ? 'Bonne réponse' : 'Mauvaise réponse'}</p>
            <p>{question['explication']}</p>
            <p className='text-lg font-semibold'>{`Votre score : ${goodAnswers}/${json.length}`}</p>
          </div>
        } 
      </div>
    </div>
